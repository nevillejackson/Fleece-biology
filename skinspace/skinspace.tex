...
The first thing we note is that if we look at the relationship of individual sheep Fas estimates and wrinkle scores we see nothing interesting. The correlation is near-zero. There is too much noise. The reason we see something with the Carter data above is that we looked at flock means, which wee meanss of around 20 animals. So what we propose to do here is to look at relationships using sire group means.  We actually list all 78 sire group means in Table~\ref{tab:smeans}.
\input{tabsmeans.tex}
